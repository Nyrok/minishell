...
static int	cd_home(t_main *main, char *oldpwd, int onlyonecommand)
{
	char	*home;
	...
	changepwd(main, oldpwd, onlyonecommand);
	main->last_exit_status = 0;
	return (1);
}

static int	cd_error_cases(t_main *main, int total_args, const char *path)
{
	if (total_args > 2)
	{
...
int	cd(t_main *main, int total_args, const char *path, int onlyonecommand)
{
	...
	if (cd_error_cases(main, total_args, path) == 0)
		return (0);
	oldpwd = get_env_value(main->envp, "PWD");
	if (total_args == 1)
		return (cd_home(main, oldpwd, onlyonecommand));
	changepwd(main, oldpwd, onlyonecommand);
